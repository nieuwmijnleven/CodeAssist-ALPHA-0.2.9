...
public class PatternTest {
  ...
  public void test() throws IOException {
    ...
    JavaFileObject javaFileObject =
        new SourceFileObject(Paths.get("test.java"), TEXT, Instant.now());
    ...
    compile.run(
        task -> {
          ...
          JavacTreePattern.Capture<LiteralTree> capture =
              literal()
                  .methodCallParameter(
                      0, method().withName("println").definedInClass("java.io.PrintStream"));
          ...
          ClassTreePattern test =
              classTree()
                  .withField(false, variable().withName("test").withType("java.lang.String"));
          assert test.accepts(root.getTypeDecls().get(0), context);

          ClassTreePattern main = classTree().withMethod(false, method().withName("main"));
