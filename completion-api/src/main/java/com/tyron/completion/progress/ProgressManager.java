...
public class ProgressManager {
   ...
   * @param cancelConsumer The code to run when this task has been canceled, called from background
   *     thread
  ...
  public void runAsync(Context uiContext, Runnable runnable, ProgressIndicator indicator) {
  ...
  /**
   * Run a non cancelable task in the background. If the task has been running for more than two
   * seconds, The loadingRunnable will be run. If the task has finished before 2000, the
   * loadingRunnable will not be called.
   *
   * @param taskToRun The task to run
   * @param loadingRunnable The runnable to run if the task has been running for more than 2 seconds
